...
class airsyncbase_Body(object):
    ...
    def marshal(self):
        import base64
        return "%s//%s//%s//%s//%s//%s" % (repr(self.airsyncbase_Type), repr(self.airsyncbase_EstimatedDataSize), repr(self.airsyncbase_Truncated), base64.b64encode(bytearray(self.airsyncbase_Data, encoding='utf-8')), repr(self.airsyncbase_Part), repr(self.airsyncbase_Preview))
    def __repr__(self):
        return self.marshal()
